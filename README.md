...
IdentiQ React FastAPI TypeScript
...
KYC Form: User fills personal details (name, DOB, gender, marital status, purpose)
Document List Generation: System generates personalized document requirements
Document Upload: User uploads required documents (PAN, Aadhaar, Passport, etc.)
Selfie Upload: User uploads live selfie video
Processing:
...
UKN Issuance: Approved users receive Unique KYC Number
Blockchain Record: UKN and verification data stored on blockchain
...
test_admin_applications.py - Admin application management
...
test_admin_metrics.py - Dashboard metrics

Metrics calculation
Status aggregation
test_kyc_user_workflow.py - User KYC workflow

Application creation
Document upload
Processing workflow
test_blockchain_records.py - Blockchain integration

Record retrieval
UKN lookup
Hash verification
test_face_dedupe.py - Face deduplication

Duplicate detection
False positive handling
test_ukn_lookup.py - UKN resolution
...
Test Files
ApiIntegration.test.ts - Frontend-Backend API integration
...
Navigation
AdminDashboard.test.tsx - Dashboard functionality

Metrics loading
Data display
ApplicationsList.test.tsx - Applications table

Data rendering
Filtering
ApplicationDetail.test.tsx - Application details

Data display
Approve/reject actions
SettingsPage.test.tsx - Settings
...
Approve without Reviewer Role → 403
Large Batch Insert (2000 applications)
Slow OCR Timeout simulation
Face Dedupe False Positive
Blockchain Hash Mismatch → 409
UKN Lookup without Consent → 403
Incorrect Schema → Validation error
Frontend
...
Deploy using Docker or cloud platform (AWS, GCP, Azure)
Example Dockerfile:
...
Update API base URL in production environment variables
...
On Windows: EasyOCR should work out of the box with the pip install.
